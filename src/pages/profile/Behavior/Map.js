...
export default function() {
  ...
  useEffect(() => {
    mapRef.current = initMap('behaviorMap');
   ...
   */
  useEffect(() => {
    if (!mapRef.current || allBehavior.length === 0) return;

    const lnglats = allBehavior
  ...
  return <div className={styles.map} id="behaviorMap" />;
}
