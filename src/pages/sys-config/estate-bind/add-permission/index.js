...
export default function() {
  ...
  const { treeData } = useSelector(({ estateBind }) => estateBind);
  const { childObj } = useSelector(({ estateBind }) => estateBind);
  ...
  useEffect(() => {
    dispatch({ type: 'estateBind/getCommunityOrg' });
    getAllUserList();
    return () => {
  ...
  const add = () => {
    dispatch({
      type: 'estateBind/authUserCommunity',
      payload: {
      ...
      callback: () => {
        router.push('/sys-config/estate-bind')
      }
    });
  };
