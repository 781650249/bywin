...
import { Collapse, Timeline, Row, Col, Avatar, message } from 'antd';
import { uniqBy } from 'lodash';
import { Viewer } from '@/components';
import { initMap, LngLat } from '@/utils/leafletMap';
import { getTrack } from '@/services/file/personnel-file';
...
export default function({ ygxxbz }) {
  ...
  const onPrev = () => {
    ...
    setLargeImgIndex((state) => state - 1);
  };
   ...
   */
  const onNext = () => {
    if (largeImgIndex === trackData.length - 1) {
      message.warn('没有图片啦!');
      return;
    }
    setLargeImgIndex((state) => state + 1);
  ...
  return (
    <Card id="track" title="轨迹记录" extra={`${trackData.length}条`}>
              ...
              <Timeline>
                {trackData.length > 0 ? (
                  trackData.map((item, index) => (
                    <Timeline.Item
                      key={index}
                      dot={<span className={styles.dot}>{index + 1}</span>}
                    >
      ...
      <Viewer
        ...
        onNext={onNext}
        relativePositions={
          trackData[largeImgIndex] ? [trackData[largeImgIndex].relativePosition] : []
        }
      />
    </Card>
  );
